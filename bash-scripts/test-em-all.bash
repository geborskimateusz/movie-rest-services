...
function testCircuitBreaker() {
    ...
    assertEqual "CLOSED" "$($EXEC wget movie-composite/actuator/health -qO - | jq -r .details.movieCircuitBreaker.details.state)"
    ...
    assertCurl 200 "curl -k https://$HOST:$PORT/movie-composite/$MOV_ID_REVS_RECS?delay=3 $AUTH -s"
    assertEqual "Fallback movie2" "$(echo "$RESPONSE" | jq -r .name)"
    ...
    assertEqual "HALF_OPEN" "$($EXEC wget movie-composite/actuator/health -qO - | jq -r .details.movieCircuitBreaker.details.state)"
    ...
    do
        assertCurl 200 "curl -k https://$HOST:$PORT/movie-composite/$MOV_ID_REVS_RECS $AUTH -s"
        assertEqual "movie name C" "$(echo "$RESPONSE" | jq -r .name)"
    done
    ...
    assertEqual "CLOSED_TO_OPEN"      "$($EXEC wget movie-composite/actuator/circuitbreakerevents/movie/STATE_TRANSITION -qO - | jq -r .circuitBreakerEvents[-3].stateTransition)"
    assertEqual "OPEN_TO_HALF_OPEN"   "$($EXEC wget movie-composite/actuator/circuitbreakerevents/movie/STATE_TRANSITION -qO - | jq -r .circuitBreakerEvents[-2].stateTransition)"
    assertEqual "HALF_OPEN_TO_CLOSED" "$($EXEC wget movie-composite/actuator/circuitbreakerevents/movie/STATE_TRANSITION -qO - | jq -r .circuitBreakerEvents[-1].stateTransition)"
